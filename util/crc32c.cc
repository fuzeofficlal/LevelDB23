...
// Modernized for C++23.
//
// A portable CRC32C implementation with optional SSE4.2 hardware acceleration.
// The SSE4.2 path is selected at compile time via #ifdef __SSE4_2__.
...
#include "util/coding.h"  // DecodeFixed32

#ifdef __SSE4_2__
#include <nmmintrin.h>  // SSE4.2 CRC32C intrinsics
#endif
...
constexpr uint32_t kByteExtensionTable[256] = {
...
constexpr uint32_t kStrideExtensionTable0[256] = {
...
constexpr uint32_t kStrideExtensionTable1[256] = {
...
constexpr uint32_t kStrideExtensionTable2[256] = {
...
constexpr uint32_t kStrideExtensionTable3[256] = {
...
inline uint32_t ReadUint32LE(const uint8_t* buffer) noexcept {
...
template <int N>
inline const uint8_t* RoundUp(const uint8_t* pointer) noexcept {
...
// ---------------------------------------------------------------------------
// SSE4.2 hardware-accelerated CRC32C (compile-time selection)
// ---------------------------------------------------------------------------
#ifdef __SSE4_2__

uint32_t HardwareExtend(uint32_t crc, const uint8_t* data, size_t n) noexcept {
  uint32_t c = crc ^ kCRC32Xor;

  // Process 4 bytes at a time.
  while (n >= 4) {
    c = _mm_crc32_u32(c, *reinterpret_cast<const uint32_t*>(data));
    data += 4;
    n -= 4;
  }
...
// ---------------------------------------------------------------------------
// Software fallback (strided table-lookup)
// ---------------------------------------------------------------------------

uint32_t Extend(uint32_t crc, std::string_view data) noexcept {
#ifdef __SSE4_2__
  return HardwareExtend(crc,
